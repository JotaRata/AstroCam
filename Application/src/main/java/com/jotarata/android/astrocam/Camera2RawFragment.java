...
public class Camera2RawFragment extends Fragment
    ...
    private CameraCaptureSession.CaptureCallback mPreCaptureCallback
        ...
        private void process(CaptureResult result, boolean finalResult) {
            synchronized (mCameraStateLock) {
                switch (mState) {
                    case STATE_PREVIEW: {
                    ...
                    case STATE_WAITING_FOR_3A_CONVERGENCE: {
                        ...
                        if (readyToCapture && mPendingUserCaptures > 0 && !waitingForSave) {
                            Log.d("process loop in", String.valueOf(waitingForSave));
                            captureStillPictureLocked();
                            // Capture once for each user tap of the "Picture" button.
                           /* while (mPendingUserCaptures > 0) {
                                Log.d("Initializing capture.. remaining", String.valueOf(mPendingUserCaptures));
                                captureStillPictureLocked();
    ...
    private final CameraCaptureSession.CaptureCallback mCaptureCallback
        ...
        public void onCaptureStarted(CameraCaptureSession session, CaptureRequest request,
           ...
           /* File directory = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DCIM),"/Astrocam_temp/");
            if (! directory.exists()){
                directory.mkdir();
            ...
            //File rawFile = new File(directory,"FITS_" + currentDateTime + ".fits");
            ...
            if (rawBuilder != null) rawBuilder.setFile(null);*/
        }
        ...
        public void onCaptureCompleted(CameraCaptureSession session, CaptureRequest request,
            ...
            if (!waitingForSave)
            {
                onCaptureFailed(session, request, null);
                return;
            }
    ...
    private int mTimerQueue = 0;

    private  MediaPlayer camera_start;
    private  MediaPlayer camera_end;
    private  MediaPlayer camera_moved;
    private  MediaPlayer camera_sucess;
    ...
    public void EnableCaptureSession(boolean enable)
    {
        ...
        if (enable == true)
        {
            mShakeWarn.setVisibility(View.INVISIBLE);
        }else
        {
            mShakeWarn.setVisibility(View.VISIBLE);
            if (timerActive)
            {
                if (mTimer != null)
                {
            ...
            if (waitingForSave) {
                ...
                mTimerQueue = mPendingUserCaptures;
                mPendingUserCaptures = 0;
                onPause();
                onResume();
                Log.e("Shake detected", String.format("Restarting capture session.. pending frames is %o", mTimerQueue));
                if (mTimer != null)
                {
                    mTimer.cancel();
                    timerActive = false;
    ...
    public void onViewCreated(final View view, Bundle savedInstanceState) {
        ...
        camera_moved = MediaPlayer.create(getActivity(), R.raw.capture_moved);
        camera_sucess = MediaPlayer.create(getActivity(), R.raw.capture_sucess);

        timer_tick1 = MediaPlayer.create(getActivity(), R.raw.timer_tick_1);
        timer_tick2 = MediaPlayer.create(getActivity(), R.raw.timer_tick_2);
        timer_tick3 = MediaPlayer.create(getActivity(), R.raw.timer_tick_3);

        mTextureView = (AutoFitTextureView) view.findViewById(R.id.texture);

        mSensorActivity = new SensorActivity(getActivity());
        mSensorActivity.SetListener(new SensorActivity.OnShakeListener() {
            @Override
            public void OnShake() {
                EnableCaptureSession(false);
                //Log.d("Sensor","Phone is shaking");
            }

            @Override
            public void OnRest() {
                EnableCaptureSession(true);
                //Log.d("Sensor","Calm");
            }
        });
    }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.picture: {
                    if (!timerActive && !waitingForSave)
                    {
                        InitializeTimerCountDown();
                        mPicture.setForeground(getResources().getDrawable(R.drawable.ic_baseline_stop_circle_24));
                    }else
                    {
                        mTimer.cancel();
                        mTimerView.setVisibility(View.INVISIBLE);
    ...
    private void CancelCapture() {
        ...
        getActivity().runOnUiThread(new Runnable() {
            @Override
            public void run() {
                mPicture.setForeground(getResources().getDrawable(R.drawable.ic_baseline_camera_24));
            }
        });
    ...
    private void InitializeTimerCountDown() {
        ...
        mTimer = new CountDownTimer(4000, 1000) {
            ...
            public void onFinish() {
                ...
                Log.d("Countdown timer finished: Number of requests is", String.valueOf(mTimerQueue));
                mTimerView.setVisibility(View.INVISIBLE);
    ...
    private void captureStillPictureLocked() {
        try {
            ...
            ImageSaver.ImageSaverBuilder rawBuilder = new ImageSaver.ImageSaverBuilder(activity)
                    .setCharacteristics(mCharacteristics).setCallback(OnImageSaved);
    ...
    private interface ImageSavedEvent{
        void run();
    }
    private  ImageSavedEvent OnImageSaved = new ImageSavedEvent() {
        @Override
        public void run() {
            mPendingUserCaptures--;
    ...
    private static class ImageSaver implements Runnable {
        ...
        public static class ImageSaverBuilder {
            ...
            public synchronized ImageSaverBuilder setCallback(final ImageSavedEvent event) {
            ...
            private boolean isComplete() {
                return mImage != null  && mCaptureResult != null
    ...
    public static String generateTimestamp() {
        SimpleDateFormat sdf = new SimpleDateFormat("HH-mm-ss(SS)--dd-MM-yyyy", Locale.US); //yyyy_MM_dd_HH_mm
